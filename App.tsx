  ...
  return (
    <div className="min-h-screen flex flex-col items-center pb-20">
      ...
      <div className="fixed top-0 left-0 right-0 p-3 md:p-6 z-40 flex items-center justify-between pointer-events-none">
        ...
        {view !== 'home' && (
          <div className="absolute left-1/2 transform -translate-x-1/2 pointer-events-auto max-w-[120px] sm:max-w-xs md:max-w-xl transition-all duration-300">
              ...
              return (
                <div key={cat.id} className="bg-white dark:bg-gray-800 rounded-2xl md:rounded-3xl shadow-xl dark:shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden transition-all duration-300">
                  <button 
                    ...
                    className="w-full flex justify-between items-center p-6 md:p-8 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-750 transition-colors text-left group"
                  >
                  ...
                  {isExpanded && (
                    <div className="p-6 md:p-8 pt-2 border-t border-gray-100 dark:border-gray-700">
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mt-4">
                        {cat.quizzes.map(quiz => {
                          ...
                          return (
                            <button key={quiz.id} onClick={() => handleStartQuiz(quiz.id)} className={`group relative bg-white dark:bg-gray-800 p-5 md:p-6 rounded-2xl md:rounded-3xl shadow-sm border-2 border-gray-200 dark:border-gray-700 transition-all duration-300 text-left hover:-translate-y-1 flex flex-col h-full ${hoverColorMap[quiz.color]}`}>
                              <div className="flex items-center gap-4 md:gap-5 mb-5 md:mb-6">
                                <div className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl flex items-center justify-center text-3xl md:text-4xl group-hover:scale-110 transition-transform ${colorMap[quiz.color].split(' ').slice(0,3).join(' ')}`}>
                                  {quiz.icon}
                                </div>
      ...
      {(view === 'quiz' || view === 'results') && currentQuizId && (
        <div className="w-full max-w-4xl px-4 py-6 md:py-8 mt-20 md:mt-20 flex-1 flex flex-col relative animate-fade-in">
                ...
                <div key={qIndex} id={`q-${qIndex}`} className={`bg-white dark:bg-gray-800 rounded-2xl md:rounded-3xl shadow-lg border ${borderColor} p-5 md:p-8 relative transition-all duration-300`}>
                   <div className="flex justify-between items-start mb-6 md:mb-8 gap-4 md:gap-6">
                      ...
                      <button 
                        ...
                        className={`flex-shrink-0 p-1 md:p-2 text-xl md:text-2xl transition-colors ${
                          isFlagged ? 'text-yellow-500' : isSubmitted ? 'text-gray-300 dark:text-gray-600' : 'text-gray-400 hover:text-yellow-500'
                        } ${isSubmitted ? 'cursor-default' : 'cursor-pointer'}`}
                      >
                        <svg className="w-5 h-5 md:w-8 md:h-8" fill="currentColor" viewBox="0 0 24 24"><path d="M 5 21 V 4 h 14 l -4 6 l 4 6 h -13 v 5 h -2 z" /></svg>
                      </button>
                   </div>
          ...
          {/* Action Button */}
          {view === 'quiz' && (
            <div className="mt-8 md:mt-12 mb-8 md:mb-10 text-center">
              <button 
                onClick={handleSubmit}
                className="group relative inline-flex items-center justify-center px-6 py-3 md:px-10 md:py-5 text-base md:text-xl font-bold text-white transition-all duration-200 bg-primary-600 rounded-2xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-600 hover:bg-primary-700 hover:shadow-xl hover:-translate-y-1"
              >
                <span>Invia Risposte</span>
                <svg className="w-5 h-5 md:w-6 md:h-6 ml-2 md:ml-3 -mr-1 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7l5 5m0 0l-5 5m5-5H6"></path></svg>
              </button>
            </div>
          )}
        </div>
